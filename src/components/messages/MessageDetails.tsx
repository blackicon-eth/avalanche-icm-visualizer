...
function Field({ label, value, copy }: { label: string; value?: string; copy?: boolean }) {
  return <div className="min-w-0"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">{label}</p><div className="mt-1 flex items-start gap-2"><p className="min-w-0 break-all text-xs leading-5">{value || "Not recorded"}</p>{copy && value && <CopyButton value={value} />}</div></div>
}
...
export function MessageDetails({ message, chains, onClose }: MessageDetailsProps) {
  ...
  return <AnimatePresence>{message && <motion.aside initial={{ opacity: 0, x: 32 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 32 }} transition={{ duration: 0.22, ease: "easeOut" }} aria-label="Message details" className="fixed inset-y-0 right-0 z-40 flex w-full max-w-xl flex-col border-l border-border bg-background shadow-2xl shadow-black/10 sm:w-[min(92vw,34rem)]">
    <header className="flex items-start justify-between gap-4 border-b border-border px-6 py-5"><div><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Message detail</p><h2 className="mt-2 break-all font-mono text-sm font-semibold">{message.id}</h2></div><button type="button" onClick={onClose} aria-label="Close message details" className="rounded-lg border border-border px-3 py-2 text-lg leading-none text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/50">×</button></header>
